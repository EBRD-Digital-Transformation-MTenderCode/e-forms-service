...
fun array(builder: ArrayDefinition.Builder.() -> Unit) = ArrayDefinition.Builder().apply(builder)
...
class ArrayDefinition(
    ...
    private fun asItems(context: Context): Map<String, Any> =
    ...
    override fun buildData(context: Context): Map<String, Any> = if (children.isNotEmpty()) {
        val attributes = getAttributes(context)
        if (attributes.isNotEmpty()) {
            linkedMapOf<String, Any>().apply {
                for (attribute in attributes) {
